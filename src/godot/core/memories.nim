...
proc `[]`*[T](p: gdptr[T]): var T = (ptr T)(p)[]
...
func decAddrBytes[T](p: gdptr[T]; bytes: uint64): gdptr[T] = cast[gdptr[T]](cast[uint64](p) - bytes)
...
proc free*[T](_: typedesc[GdMemory]; p_ptr: gdptr[T]; p_pad_align= false) =
  ...
  if prepad: mem = mem.decAddrBytes PAD_ALIGN

  gdinterface.mem_free(cast[pointer](mem))
