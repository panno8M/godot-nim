...
proc isNormalized*(self: NoiseTexture3D): Bool =
  ...
  interface_Object_methodBindPtrCall(methodbind, getOwner self, nil, addr ret)
  (addr ret).decode_result(Bool)
proc `colorRamp=`*(self: NoiseTexture3D; gradient: Gradient) =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
  ...
  interface_Object_methodBindPtrCall(methodbind, getOwner self, addr `?param`[0], nil)
proc colorRamp*(self: NoiseTexture3D): Gradient =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
...
proc `noise=`*(self: NoiseTexture3D; noise: Noise) =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
  ...
  interface_Object_methodBindPtrCall(methodbind, getOwner self, addr `?param`[0], nil)
proc noise*(self: NoiseTexture3D): Noise =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
