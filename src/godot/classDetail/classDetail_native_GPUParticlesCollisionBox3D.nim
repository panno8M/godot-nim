...
proc `size=`*(self: GPUParticlesCollisionBox3D; size: Vector3) =
  ...
  if unlikely(methodbind.isNil):
    ...
    methodbind = interface_ClassDB_getMethodBind(addr className GPUParticlesCollisionBox3D, addr name, 3460891852)
...
proc size*(self: GPUParticlesCollisionBox3D): Vector3 =
  ...
  if unlikely(methodbind.isNil):
    ...
    methodbind = interface_ClassDB_getMethodBind(addr className GPUParticlesCollisionBox3D, addr name, 3360562783)
