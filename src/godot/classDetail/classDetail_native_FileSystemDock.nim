...
proc addResourceTooltipPlugin*(self: FileSystemDock; plugin: EditorResourceTooltipPlugin) =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
  ...
  interface_Object_methodBindPtrCall(methodbind, getOwner self, addr `?param`[0], nil)
proc removeResourceTooltipPlugin*(self: FileSystemDock; plugin: EditorResourceTooltipPlugin) =
  var methodbind {.global.}: MethodBindPtr
  if unlikely(methodbind.isNil):
